...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def update_weights(self, global_steps: int = None):
        ...
        """
        raw_hf_config = getattr(self.actor.engine.model_config, "hf_config", None)
        dflash_enabled = bool(getattr(raw_hf_config, "verl_composed_dflash_student", False))
        eagle3_enabled = bool(getattr(raw_hf_config, "verl_composed_eagle3_student", False))
        if dflash_enabled and eagle3_enabled:
            raise ValueError("Only one composed OPD draft student can be enabled: DFLASH or EAGLE3.")
        dflash_draft_only = dflash_enabled or eagle3_enabled

        # 0. send_weights only for async training with disaggregated trainer and rollout
        if self.config.rollout.checkpoint_engine.backend != "naive":
            per_tensor_param, _ = self.actor.engine.get_per_tensor_param(trainable_only=dflash_draft_only)
        ...
        per_tensor_param, peft_config = self.actor.engine.get_per_tensor_param(
            layered_summon=self.layered_summon, base_sync_done=True, trainable_only=dflash_draft_only
        )
        ...
        if do_lora_base_sync:
            per_tensor_param_base, peft_config = self.actor.engine.get_per_tensor_param(
                layered_summon=self.layered_summon, base_sync_done=False, trainable_only=dflash_draft_only
            )
            await self.rollout.update_weights(
                ...
                dflash_draft_only=dflash_draft_only,
            )

        sync_stats = await self.rollout.update_weights(
            per_tensor_param,
            peft_config=peft_config,
            base_sync_done=True,
            global_steps=global_steps,
