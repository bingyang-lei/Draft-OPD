...
def align_dflash_reject_token_mask(raw_mask: Any, response_len: int) -> list[bool]:
    """Align SGLang DFLASH verify-step metadata to response token positions.

    SGLang appends the first response token during prefill before DFLASH verify
    starts, so current DFLASH metadata usually has length ``response_len - 1``.
    Keep this tolerant of future SGLang versions that may already emit a
    full-length mask.
    """
    if response_len <= 0:
        return []
    ...
    elif hasattr(raw_mask, "tolist"):
        raw_values = [bool(item) for item in raw_mask.tolist()]
    else:
        raw_values = [bool(item) for item in raw_mask]

    if len(raw_values) == response_len:
        aligned = raw_values[:response_len]
    else:
        aligned = [False] + raw_values
        aligned = aligned[:response_len]
        if len(aligned) < response_len:
            aligned.extend([False] * (response_len - len(aligned)))
    return aligned
