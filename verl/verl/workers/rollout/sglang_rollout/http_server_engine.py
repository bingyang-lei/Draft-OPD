...
class HttpServerAdapter(EngineBase):
    ...
    def update_weights_from_tensor(self, req: UpdateWeightsFromTensorReqInput) -> dict[str, Any]:
        ...
        draft_model_only = getattr(req, "draft_model_only", None)
        ...
        if draft_model_only is not None:
            payload["draft_model_only"] = draft_model_only
...
class AsyncHttpServerAdapter(HttpServerAdapter):
    ...
    async def _make_async_request(
        ...
        detail = f": last error was {type(last_error).__name__}: {last_error}" if last_error is not None else ""
        raise RuntimeError(
            f"Failed to complete async request to {endpoint} after {self.max_attempts} attempts{detail}"
        )
