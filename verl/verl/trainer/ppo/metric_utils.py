...
def _numeric_non_tensor_values(batch: DataProto, key: str) -> np.ndarray | None:
...
def compute_dflash_rollout_metrics(batch: DataProto) -> dict[str, Any]:
    metrics = {}
    for key, metric_prefix in (
        ("dflash_reject_token_mask_raw_len", "opd/rollout/reject_mask_raw_len"),
    ...
    empty_values = _numeric_non_tensor_values(batch, "dflash_empty_reject_token_indices")
    if empty_values is not None:
        metrics["opd/rollout/empty_reject_ratio"] = float(np.mean(empty_values))
    return metrics
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            - critic/score/mean, max, min: Statistics about sequence scores (if use_critic=True)
            - critic/rewards/mean, max, min: Statistics about sequence rewards (if use_critic=True)
            - critic/advantages/mean, max, min: Statistics about advantages (if use_critic=True)
            - critic/returns/mean, max, min: Statistics about returns (if use_critic=True)
    ...
    metrics.update(compute_dflash_rollout_metrics(batch))
...
def compute_sglang_spec_accept_length(gen_batch_output: DataProto) -> float | None:
    """Compute only speculative accept length for rollout speed-test logging."""
    non_tensor_batch = gen_batch_output.non_tensor_batch
    completion_tokens = non_tensor_batch.get("sglang_completion_tokens")
    spec_verify_ct = non_tensor_batch.get("sglang_spec_verify_ct")
    if completion_tokens is None or spec_verify_ct is None:
        return None

    total_verify_ct = 0.0
    total_accepted_draft_tokens = 0.0
    accepted_draft_tokens = non_tensor_batch.get("sglang_spec_accept_token_num")
    if accepted_draft_tokens is not None:
        for accept_value, verify_value in zip(accepted_draft_tokens, spec_verify_ct, strict=False):
            accept_value = _to_optional_float(accept_value)
            verify_value = _to_optional_float(verify_value)
            if accept_value is None or verify_value is None or verify_value <= 0:
                continue
            total_accepted_draft_tokens += accept_value
            total_verify_ct += verify_value
    else:
        for token_value, verify_value in zip(completion_tokens, spec_verify_ct, strict=False):
            token_value = _to_optional_float(token_value)
            verify_value = _to_optional_float(verify_value)
            if token_value is None or verify_value is None or verify_value <= 0:
                continue
            total_accepted_draft_tokens += max(token_value - verify_value, 0.0)
            total_verify_ct += verify_value

    if total_verify_ct <= 0:
        return None
    return total_accepted_draft_tokens / total_verify_ct


def compute_sglang_rollout_metrics(gen_batch_output: DataProto, timing_raw: dict[str, float]) -> dict[str, Any]:
    """Compute batch-level SGLang speculative rollout metrics."""
    non_tensor_batch = gen_batch_output.non_tensor_batch
    completion_tokens = non_tensor_batch.get("sglang_completion_tokens")
    ...
    for value in completion_tokens:
        value = _to_optional_float(value)
        if value is not None and value >= 0:
            total_completion_tokens += value

    metrics = {}
    gen_time = timing_raw.get("gen")
    if total_completion_tokens > 0 and gen_time is not None and gen_time > 0:
        metrics["rollout/sglang_throughput"] = total_completion_tokens / gen_time
    ...
    if accepted_draft_tokens is not None:
        ...
        for token_value, verify_value in zip(completion_tokens, spec_verify_ct, strict=False):
            ...
            if token_value is None or verify_value is None or verify_value <= 0:
                continue
            # Older SGLang metadata only exposes completion_tokens. Subtract one
            # target-produced boundary token per verify step to approximate the
            # number of accepted draft tokens.
            total_accepted_draft_tokens += max(token_value - verify_value, 0.0)
            total_verify_ct += verify_value

    if total_verify_ct > 0:
        metrics["rollout/sglang_spec_accept_length"] = total_accepted_draft_tokens / total_verify_ct

    draft_token_count = non_tensor_batch.get("sglang_spec_draft_token_num")
    if draft_token_count is None:
        return metrics

    total_draft_tokens = 0.0
    for value in draft_token_count:
        value = _to_optional_float(value)
        if value is not None and value > 0:
            total_draft_tokens += value

    if total_draft_tokens > 0:
        metrics["rollout/sglang_spec_accept_rate"] = total_accepted_draft_tokens / total_draft_tokens
    return metrics
