...
def extract_reward(batch: DataProto):
    ...
    if "rm_scores" in batch.batch.keys():
        ...
        # Distillation-only training may disable task rewards entirely. In that case
        # no reward loop / RM path populates rm_scores, but downstream PPO plumbing
        # still expects a token-level reward tensor. Use an all-zero reward tensor so
        # the policy-reward term becomes a no-op while distillation loss still trains.
        if "response_mask" not in batch.batch.keys():
            raise KeyError(
                'Neither "rm_scores" nor "response_mask" found in batch; cannot construct fallback reward tensor.'
            )
