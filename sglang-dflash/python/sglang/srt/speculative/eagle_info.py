...
class EagleVerifyInput(SpecInput, EagleVerifyInputV2Mixin):
    ...
    def verify(
        ...
        for i, (req, accept_index_row) in enumerate(zip(batch.reqs, accept_index_cpu)):
            ...
            if batch.spec_algorithm.is_eagle3():
                append_dflash_reject_token_mask(
                    req,
                    num_accepted,
                    output_ids_already_updated=True,
                )
                max_rejected_offset = self.draft_token_num - 1
                if self.topk == 1 and num_accepted >= accepted_draft_tokens + 1 and accepted_draft_tokens < max_rejected_offset:
                    start = max(0, int(accepted_draft_tokens))
                    end = int(max_rejected_offset)
                    rejected_token_tensor = candidates[i, start:end]
                    if rejected_token_tensor.numel() > 0:
                        row_base = i * self.draft_token_num
                        ...
                        rejected_log_probs = F.log_softmax(
                            logits_output.next_token_logits[row_indices].float(),
                            dim=-1,
                        ).gather(dim=-1, index=rejected_token_ids.unsqueeze(-1)).squeeze(-1)
                        append_dflash_rejected_draft_metadata(
                            req,
                            num_accepted,
                            anchor_index=len(req.output_ids) - num_accepted - 1,
                            offsets=list(range(start + 1, self.draft_token_num)),
                            token_ids=rejected_token_tensor.tolist(),
                            teacher_logprobs=rejected_log_probs.detach().cpu().tolist(),
                            output_ids_already_updated=True,
                        )
