...
class DFlashVerifyInput(SpecInput):
    ...
    def verify(
        ...
        *,
        ...
        # Single D2H transfer: candidates[1:] + accept_len + bonus + teacher logprobs.
        packed = torch.cat(
            [candidates[:, 1:], accept_len.unsqueeze(1), bonus.unsqueeze(1)], dim=1
        ).cpu()
        packed_candidate_teacher_logprobs = candidate_teacher_logprobs.cpu()
        ...
        for i, req in enumerate(batch.reqs):
            ...
            if reached_rejection_point and acc_len < max_acc:
                ...
                rejected_token_ids = [
                    int(token_id) for token_id in packed[i, acc_len:max_acc].tolist()
                ]
                rejected_teacher_logprobs = [
                    float(logprob)
                    for logprob in packed_candidate_teacher_logprobs[
                        i, acc_len:max_acc
                    ].tolist()
                ]
                append_dflash_rejected_draft_metadata(
